...
RETURNS TABLE(s integer, avail boolean) AS $$
DECLARE
    sta RECORD;
BEGIN
    FOR sta in SELECT * from station left join station_route sr on station.station_id = sr.station_id
    LOOP
        select sta.station_id into s;
        IF sta.is_stop THEN
            avail := True;
        END IF;
        IF sta.is_stop != True THEN
            avail := False;
        end if;
    END LOOP;
END;
