...
DROP FUNCTION IF EXISTS truncate_tables(t_name VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS export_station();
DROP FUNCTION IF EXISTS export_railline();
DROP FUNCTION IF EXISTS export_station_line();
DROP FUNCTION IF EXISTS export_route();
DROP FUNCTION IF EXISTS export_station_route();
DROP FUNCTION IF EXISTS export_train();
DROP FUNCTION IF EXISTS export_train_schedule();
DROP FUNCTION IF EXISTS export_passenger();
DROP FUNCTION IF EXISTS export_reservation();
DROP FUNCTION IF EXISTS station_all_pass();
DROP FUNCTION IF EXISTS pass_same_stop_not_same(rt int) CASCADE;
DROP FUNCTION IF EXISTS find_availability(dd varchar(10), tt_start time, tt_end time) CASCADE;
...
--- truncate tables  --
CREATE OR REPLACE FUNCTION truncate_tables(t_name IN VARCHAR) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION export_station() RETURNS TABLE(station_id INTEGER, name VARCHAR(30), opening_time TIME,
closing_time TIME, delay INTEGER, street VARCHAR(100), town VARCHAR(100), postal_code CHAR(13)) AS $$
    SELECT * FROM station;
$$ LANGUAGE sql;

--export railline data --
CREATE OR REPLACE FUNCTION export_railline() RETURNS TABLE(rail_id INTEGER, speed_limit VARCHAR(30)) AS $$
    SELECT * FROM railline;
$$ LANGUAGE sql;

--export station line data --
CREATE OR REPLACE FUNCTION export_station_line() RETURNS TABLE(rail_id INTEGER, station_id INTEGER,
distance_prev INTEGER, order_in_line INTEGER) AS $$
    SELECT * FROM station_line;
$$ LANGUAGE sql;

--export route data --
CREATE OR REPLACE FUNCTION export_route() RETURNS TABLE(route_id INTEGER) AS $$
    SELECT * FROM route;
$$ LANGUAGE sql;


-- export station route data --
CREATE OR REPLACE FUNCTION export_station_route() RETURNS TABLE(route_id INTEGER, station_id INTEGER,
is_stop BOOLEAN, order_in_route INTEGER) AS $$
    SELECT * FROM station_route;
$$ LANGUAGE sql;


--export train data --
CREATE OR REPLACE FUNCTION export_train() RETURNS TABLE(train_id INTEGER, name VARCHAR(30),
description VARCHAR(100), num_seats INTEGER, top_speed INTEGER, price_per_km INTEGER) AS $$
    SELECT * FROM train;
$$ LANGUAGE sql;

--export train schedule --
CREATE OR REPLACE FUNCTION export_train_schedule() RETURNS TABLE(schedule_id INTEGER, route_id INTEGER,
day VARCHAR(10), t TIME, train_id INTEGER, dsrp BOOLEAN) AS $$
    SELECT * FROM trainschedule;
$$ LANGUAGE sql;

--export passenger data --
CREATE OR REPLACE FUNCTION export_passenger() RETURNS TABLE(customer_id INTEGER, f_name VARCHAR(20), l_name VARCHAR(30),
street_address VARCHAR(50), city VARCHAR(30), postal_code CHAR(13)) AS $$
    SELECT * FROM passenger;
$$ LANGUAGE sql;

-- export reservation data --
CREATE OR REPLACE FUNCTION export_reservation() RETURNS TABLE(reservation_id INTEGER, customer_id INTEGER,
train_sch_id INTEGER, r_start_time TIMESTAMP, rend_time INTEGER, ticketed BOOLEAN, no_adjustments bool) AS $$
BEGIN
    SELECT * FROM reservation;
END;
$$ LANGUAGE plpgsql;

--find stations that all train pass through--
CREATE OR REPLACE FUNCTION station_all_pass()
RETURNS TABLE(sta_id int) AS $$
SELECT a.sid
...
SELECT DISTINCT route_id1 FROM ((SELECT route_id, is_stop, station_id
                        FROM station_route as sx
                        WHERE (NOT EXISTS (
                        (SELECT p.station_id FROM (SELECT sp.station_id FROM station_route sp WHERE sp.route_id = rt) as p)
                        EXCEPT
                        (SELECT ssp.station_id
                        FROM station_route as ssp
                        WHERE ssp.route_id = sx.route_id)
                        )) and route_id != rt) R1(route_id1, is_stop1, station_id1)
                        CROSS JOIN
                        (SELECT route_id, is_stop, station_id
                        FROM station_route as sx
                        WHERE (NOT EXISTS (
                        (SELECT p.station_id FROM (SELECT sp.station_id FROM station_route sp WHERE sp.route_id = rt) as p)
                        EXCEPT
                        ...
                        FROM station_route as ssp
                        WHERE ssp.route_id = sx.route_id)
                        )) and route_id = rt) R2(route_id2, is_stop2, station_id2))
                        AS R3(route_id1, is_stop1, station_id1, route_id2, is_stop2, station_id2)
WHERE station_id1 = station_id2 and is_stop1 != is_stop2;
$$ LANGUAGE SQL;

 -- find availability --
CREATE OR REPLACE FUNCTION find_availability(dd varchar(10), tt_start time, tt_end time)
RETURNS TABLE(s integer, availability boolean) AS $$
DECLARE
    count_row INTEGER;
    sta INTEGER;
BEGIN
    DROP TABLE IF EXISTS out_table;
    CREATE TABLE out_table(sta_id int, availability boolean);
    FOR sta in SELECT station_id FROM station
    LOOP
--         INSERT INTO out_table VALUES (sta, FALSE);
        SELECT count(*) INTO count_row FROM trains_at_station(sta, dd, tt_start, tt_end);
        IF count_row > 0 THEN
            INSERT INTO out_table VALUES (sta, True);
        ELSE
            INSERT INTO out_table VALUES (sta, FALSE);
        END IF;
    END LOOP;
